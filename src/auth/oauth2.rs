...
pub async fn authenticate(creds: &OAuthAppCredentials) -> Result<GoogleTokens> {
    ...
    let listener = TcpListener::bind("127.0.0.1:0")
        .await
        .context("Impossible de lier un port local")?;
    ...
    println!(
        "Si le navigateur ne s'ouvre pas, visitez ce lien :\n{}\n",
        auth_url
    );
    ...
    let expires_in = token_result
        .expires_in()
        .unwrap_or(std::time::Duration::from_secs(3599))
        .as_secs() as i64;
...
mod tests {
    ...
    fn test_token_refresh_margin() {
        ...
        assert!(
            needs_refresh,
            "Le rafraîchissement doit être déclenché (marge < 60s)"
        );
    }
}
