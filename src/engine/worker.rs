...
async fn sync_file<K: KioOps>(
    ...
    let remote = to_remote(&cfg.sync_pairs[0].remote_folder_id, &cfg.sync_pairs[0].local_path, path)?;
...
async fn rename<K: KioOps>(
    ...
    let from_remote = to_remote(&cfg.sync_pairs[0].remote_folder_id, &cfg.sync_pairs[0].local_path, from)?;
    let to_remote   = to_remote(&cfg.sync_pairs[0].remote_folder_id, &cfg.sync_pairs[0].local_path, to)?;
